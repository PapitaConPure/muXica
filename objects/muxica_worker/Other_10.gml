...
while not ds_queue_empty(MUX_P_FADE) {
	...
	if MUX_SHOW_LOG_INFO
		show_debug_message($"Concluding crossfade from {is_undefined(_request.out) ? "any" : audio_get_name(_request.out)} to {audio_get_name(_request.in)} in {_request.time * 0.001}s");
}
